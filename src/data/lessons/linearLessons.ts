...
export const LINEAR_LESSONS: Lesson[] = [
  {
    ...
    overview: 'Imagine a neat row of numbered storage lockers in a memory bank, indexed 0, 1, 2, 3... Because every locker sits directly adjacent to the previous one with zero gaps in between (contiguous memory), accessing element #5 takes exactly 1 step (\\mathcal{O}(1) random access). But what happens when the allocated storage capacity fills up? The memory manager allocates a brand-new contiguous block with double the capacity (e.g. 4 → 8 → 16), moves the existing elements over, and frees the old block. That is the foundational mechanism behind a Vector (Dynamic Array)!',
    timeComplexity: {
    ...
    keyConcepts: [
      ...
      },
      {
        ...
        mathFormula: `For a 3D Array $A[L_1 : U_1, L_2 : U_2, L_3 : U_3]$:
- Lower Bounds: $L_1, L_2, L_3$ | Upper Bounds: $U_1, U_2, U_3$
- Dimension Spans (Sizes): $D_1 = U_1 - L_1 + 1, \\quad D_2 = U_2 - L_2 + 1, \\quad D_3 = U_3 - L_3 + 1$
- Element Size: $W$ words/bytes | Base Address: $\\text{Base}$
    ...
    examQuestions: [
      {
        ...
        solution: `Let's perform a rigorous step-by-step algebraic derivation:
...
- Extra Memory (Space): O(1) because we only use one helper variable (max_from_right).`,
    ...
    quizzes: [
      ...
      {
        id: 'vec-q3',
        question: 'What is the time complexity of inserting an element at the middle index (index N/2) of a vector of size N?',
        options: ['O(1)', 'O(log N)', 'O(N)', 'O(N²)'],
        correctIndex: 2,
        explanation: 'Inserting into the middle requires shifting all subsequent N/2 elements one slot to the right, which takes O(N) linear time.',
        examTip: 'Vector push_back is O(1) amortized, but mid-array insert/erase operations take O(N) due to element shifting.'
      },
      {
        id: 'vec-q4',
        question: 'What is the main purpose of calling vec.reserve(1000) on a C++ std::vector before inserting items?',
        options: [
          'It fills the vector with 1000 zero elements',
          'It pre-allocates memory capacity for 1000 items upfront to prevent repeated intermediate reallocations',
          'It sets the maximum size limit so no more than 1000 items can ever be added',
          'It sorts the vector elements automatically'
        ],
        correctIndex: 1,
        explanation: 'reserve(K) allocates memory capacity ahead of time, ensuring that push_back operations will not trigger costly memory reallocations and copies.',
        examTip: 'Use reserve() when the total count of elements is known in advance to maximize insertion performance.'
      },
      {
        id: 'vec-q5',
        question: 'In Row-Major order for a 2D array A[1..R, 1..C], which index moves fastest as elements are stored consecutively in memory?',
        options: [
          'The row index (first index)',
          'The column index (second index)',
          'Both indices move at equal speeds',
          'Neither, elements are stored randomly'
        ],
        correctIndex: 1,
        explanation: 'In Row-Major order, row 1 elements are stored for all columns before row 2 starts, so the column index (last index) changes fastest.',
        examTip: 'Row-Major = last index moves fastest; Column-Major = first index moves fastest.'
      }
    ]
  },
  {
    ...
    keyConcepts: [
      {
        ...
        description: 'In algorithmic examinations, linked lists are classified based on node linkage topologies:',
        bulletPoints: [
      ...
      },
      {
        ...
        mathFormula: `Polynomial Term Node Memory Representation:
$$ [\\text{COEF} \\mid \\text{EXP} \\mid \\text{LINK}] $$

Example: $P(x) = 4x^3 + 3x^2 - 5x + 7$
$$ [4 \\mid 3] \\to [3 \\mid 2] \\to [-5 \\mid 1] \\to [7 \\mid 0] \\to \\text{HEADER} $$`,
        bulletPoints: [
...
void deleteNodeWithoutHead(struct PolyNode* target) {
    ...
    // In-place node mutation: Copy the next node's data into target!
...
void splitUniqueAndDuplicates(struct Node* head, struct Node** head_unique, struct Node** head_dup) {
    ...
    *head_dup = NULL;
    ...
    quizzes: [
      ...
      {
        id: 'list-q2',
        question: 'Given a pointer `p` to an internal node in a Doubly Linked List, what is the time complexity to delete node `p`?',
        options: ['O(1)', 'O(log N)', 'O(N)', 'O(N²)'],
        correctIndex: 0,
        explanation: 'In a Doubly Linked List, node `p` knows both `p->prev` and `p->next`. Unlinking `p` is done in O(1) via `p->prev->next = p->next; p->next->prev = p->prev;`.',
        examTip: 'Doubly Linked Lists allow O(1) deletion given a node pointer, whereas Singly Linked Lists need O(N) to locate the predecessor.'
      },
      {
        id: 'list-q3',
        question: 'In polynomial representation using linked lists, what fields does each term node typically store?',
        options: [
          'Only coefficient (COEF)',
          'Coefficient (COEF), Exponent (EXP), and Next Pointer (LINK)',
          'Base Address and Dimension Spans',
          'Left Child, Right Child, and Balance Factor'
        ],
        correctIndex: 1,
        explanation: 'Each term node represents a monomial $c x^e$ by storing its coefficient `COEF`, its exponent `EXP`, and a `LINK` pointer to the next term in descending exponent order.',
        examTip: 'Polynomial addition algorithms scan both lists in parallel, comparing EXP values to merge or append terms.'
      },
      {
        id: 'list-q4',
        question: 'In static array implementation of linked lists, what is the AVAIL list?',
        options: [
          'A list of active user data nodes',
          'A stack-like free pool of unused available memory slots ready for allocation',
          'An array of head pointers for hash buckets',
          'A priority queue of CPU tasks'
        ],
        correctIndex: 1,
        explanation: 'AVAIL maintains all currently free array slots linked together. Allocating a node pops from AVAIL, and freeing a node pushes it back onto AVAIL.',
        examTip: 'AVAIL = Available pool of unused nodes managed as a free-storage list.'
      },
      {
        id: 'list-q5',
        question: 'What is the minimal auxiliary space complexity required to reverse a Singly Linked List in-place using three pointers (prev, curr, next)?',
        options: ['O(1)', 'O(log N)', 'O(N)', 'O(N log N)'],
        correctIndex: 0,
        explanation: 'Iterative pointer reversal only re-links `curr->next = prev` while advancing pointers, consuming O(1) extra space.',
        examTip: 'In-place linked list reversal requires O(1) extra memory, unlike array reversal which may copy buffers.'
      }
    ]
  },
  {
    ...
    overview: 'Think of a physical tray dispenser or execution call stack in a CPU! When new frames are pushed, they sit on the TOP of the stack. When a function finishes or a frame is popped, it is removed from the TOP. The very first frame pushed at the bottom is the last one removed. In computer science, this is called LIFO (Last-In, First-Out). Elements can only be pushed (inserted) or popped (removed) from the single active boundary called \`TOP\`.',
    timeComplexity: {
...
Final Answer: Value = 7`,
    ...
    quizzes: [
      ...
      {
        id: 'stk-q3',
        question: 'In converting infix expression `A + B * C` to postfix using the Shunting-Yard algorithm, what is the output sequence?',
        options: ['A B C + *', 'A B C * +', '+ A * B C', 'A B + C *'],
        correctIndex: 1,
        explanation: 'Multiplication `*` has higher precedence than addition `+`, so `B * C` is evaluated first to `B C *`, resulting in `A B C * +`.',
        examTip: 'Higher precedence operators are popped and output before lower precedence operators.'
      },
      {
        id: 'stk-q4',
        question: 'Which software runtime mechanism relies directly on a Call Stack to track return addresses and local variables?',
        options: ['Direct Memory Access (DMA)', 'Function Recursion', 'Disk I/O Interrupts', 'Garbage Collection Sweep'],
        correctIndex: 1,
        explanation: 'Function recursion pushes active function call stack frames onto the system call stack. Reaching the base case pops frames back off.',
        examTip: 'Unbounded recursion without a proper base case leads to Stack Overflow.'
      },
      {
        id: 'stk-q5',
        question: 'What is the worst-case time complexity of Push and Pop operations in a singly linked list implementation of a Stack?',
        options: ['O(1)', 'O(log N)', 'O(N)', 'O(N²)'],
        correctIndex: 0,
        explanation: 'Pushing and popping at the HEAD node of a singly linked list only updates the `head` pointer and node link, taking O(1) constant time.',
        examTip: 'Always insert and delete at the HEAD node when implementing a stack with a linked list for O(1) performance.'
      }
    ]
  },
  {
    ...
    overview: 'Think of network packets queuing at a router interface or print jobs sent to a print spooler. The very first packet to enter the buffer is the first one processed and dispatched. New packets append to the back (\`REAR\`). In computer science, this is called FIFO (First-In, First-Out). Elements are enqueued at the \`REAR\` and dequeued from the \`FRONT\`.',
    timeComplexity: {
      access: '$\\mathcal{O}(N)$',
      search: '$\\mathcal{O}(N)$',
    ...
    keyConcepts: [
      ...
      },
      {
        ...
        mathFormula: `Circular Queue Index Wrap Arithmetic:
$$ \\text{next\\_index} = (\\text{curr\\_index} + 1) \\bmod \\text{MAX} $$

Queue State Conditions:
- Empty Condition: $\\text{FRONT} = -1 \\text{ and } \\text{REAR} = -1$
- Full Condition: $(\\text{REAR} + 1) \\bmod \\text{MAX} = \\text{FRONT}$`,
        bulletPoints: [
...
Step 4: Return ITEM.`,
    ...
    quizzes: [
      ...
      {
        id: 'que-q2',
        question: 'When dequeuing the last remaining element from a Circular Queue (where FRONT == REAR), what values should FRONT and REAR be reset to?',
        options: ['FRONT = 0, REAR = 0', 'FRONT = -1, REAR = -1', 'FRONT = MAX-1, REAR = MAX-1', 'They remain unchanged'],
        correctIndex: 1,
        explanation: 'When the last item is removed, the queue becomes completely empty, so FRONT and REAR are both reset to -1.',
        examTip: 'Always check if FRONT == REAR before dequeuing to reset pointers to -1.'
      },
      {
        id: 'que-q3',
        question: 'Which Double-Ended Queue (Deque) variation allows insertions at both ends, but restricts deletion to only one end?',
        options: ['Input-Restricted Deque', 'Output-Restricted Deque', 'Circular Priority Queue', 'Monotonic Stack'],
        correctIndex: 1,
        explanation: 'An Output-Restricted Deque allows inserting elements at both FRONT and REAR, but permits removing elements from only ONE designated end.',
        examTip: 'Input-Restricted = insertion at 1 end only; Output-Restricted = deletion at 1 end only.'
      },
      {
        id: 'que-q4',
        question: 'How many total elements are currently stored in a Circular Queue of capacity N if FRONT = 4 and REAR = 1 (where 0 <= FRONT, REAR < N and N = 8)?',
        options: ['3', '5', '6', '7'],
        correctIndex: 1,
        explanation: 'Formula for count when REAR < FRONT: Count = (REAR - FRONT + N) % N + 1. Here (1 - 4 + 8) % 8 + 1 = 5 % 8 + 1 = 6 items, or (1 - 4 + 8) = 5 items stored.',
        examTip: 'Count = (REAR - FRONT + N) % N + 1 when non-empty.'
      },
      {
        id: 'que-q5',
        question: 'Why does a simple linear array queue suffer from "False Overflow"?',
        options: [
          'Because the array memory gets corrupted',
          'Because REAR reaches the end index N-1 while free slots exist at the front due to previous dequeues',
          'Because items are stored out of order',
          'Because FRONT becomes greater than REAR permanently'
        ],
        correctIndex: 1,
        explanation: 'In a linear non-circular array queue, REAR can hit N-1 (raising overflow) even if FRONT has advanced and left empty slots at indices 0, 1, 2... Circular queues fix this by wrapping REAR back to index 0.',
        examTip: 'Circular queues solve False Overflow using modulo arithmetic `(REAR + 1) % N`.'
      }
    ]
  },
  {
    ...
    keyConcepts: [
      {
        ...
        mathFormula: `Binary Heap 0-Based Array Formulas (for node at array index $i$):
- Parent Index: $\\text{parent}(i) = \\lfloor (i - 1) / 2 \\rfloor$
- Left Child Index: $\\text{left}(i) = 2i + 1$
- Right Child Index: $\\text{right}(i) = 2i + 2$
- Leaf Nodes Start Index: $\\lfloor N / 2 \\rfloor$`,
        bulletPoints: [
...
- Step 3: Swap arr[0] (55) and arr[5] (44). Heap size becomes 5.
  ...
  State 3: [55, 50, 44, 22, 30 | 55, 60, 77]`,
    ...
    quizzes: [
      ...
      {
        id: 'hp-q2',
        question: 'In a zero-indexed array representation of a binary max-heap, for a node at index `i = 4`, what is the index of its parent node?',
        options: ['1', '2', '3', '8'],
        correctIndex: 0,
        explanation: 'The parent index formula for 0-indexed heaps is `(i - 1) / 2`. For `i = 4`, parent = `(4 - 1) / 2 = 3 / 2 = 1` (integer division).',
        examTip: '0-indexed heap math: Parent = (i - 1) / 2, Left = 2i + 1, Right = 2i + 2.'
      },
      {
        id: 'hp-q3',
        question: 'When performing `extract-max` (pop) on a Max-Heap of size N, what element replaces the root before running sift-down (heapify)?',
        options: ['The smallest element in the left subtree', 'The element at index N-1 (the last leaf in the array)', 'Zero', 'The right child of the root'],
        correctIndex: 1,
        explanation: 'To maintain the complete binary tree shape, the last element at index N-1 is swapped into the root position, and then sifted down to its correct level.',
        examTip: 'Extract-max swaps root with last leaf, decrements heap size, and calls heapify(root).'
      },
      {
        id: 'hp-q4',
        question: 'What is the overall worst-case time complexity of Heapsort to sort N elements in ascending order?',
        options: ['O(N)', 'O(N log N)', 'O(N²)', 'O(2^N)'],
        correctIndex: 1,
        explanation: 'Building the max-heap takes O(N) time, and extracting N elements each taking O(log N) time yields O(N log N) total time in all cases (worst, average, best).',
        examTip: 'Heapsort guarantees O(N log N) worst-case time with O(1) auxiliary space (in-place).'
      },
      {
        id: 'hp-q5',
        question: 'Which of the following array arrangements forms a valid Max-Heap?',
        options: [
          '[10, 20, 15, 5, 8]',
          '[90, 80, 70, 60, 50, 40]',
          '[5, 10, 15, 20, 25]',
        ...
        correctIndex: 1,
        explanation: 'In [90, 80, 70, 60, 50, 40], root 90 > children (80, 70); 80 > (60, 50); 70 > 40. Every parent is strictly greater than or equal to its children.',
        examTip: 'Max-heap condition: A[parent] >= A[left_child] and A[parent] >= A[right_child] for all valid indices.'
      }
    ]
  },
  {
    id: 'set',
    categoryId: 'linear',
    subCategoryId: 'associative-hash',
    ...
    keyConcepts: [
      {
        ...
        description: 'C++ standard library provides two primary associative set implementations:',
        bulletPoints: [
      ...
      },
      {
        ...
        mathFormula: `Binary Search Tree (BST) Ordering Invariant:
$$ \\forall x \\in \\text{LeftSubtree}(u), \\quad \\text{key}(x) < \\text{key}(u) < \\forall y \\in \\text{RightSubtree}(u), \\quad \\text{key}(y) $$

Balanced Height Bound:
...
Because every single key had a hash value of 1, they formed one giant continuous block of occupied slots from index 1 to index 7! Quadratic probing avoids this by taking bigger jumping steps.`,
    ...
    quizzes: [
      ...
      {
        id: 'set-q2',
        question: 'What is the average case search time complexity of `std::unordered_set` compared to `std::set`?',
        options: [
          'unordered_set: O(1) avg | set: O(log N)',
          'unordered_set: O(log N) | set: O(1) avg',
          'unordered_set: O(N) | set: O(N)',
          'unordered_set: O(1) avg | set: O(N)'
        ],
        correctIndex: 0,
        explanation: 'std::unordered_set uses hash buckets giving O(1) average lookup, whereas std::set uses a Red-Black BST giving O(log N) guaranteed lookup.',
        examTip: 'Use unordered_set for maximum speed when key ordering does not matter.'
      },
      {
        id: 'set-q3',
        question: 'In Quadratic Probing for open addressing hash tables with function $h(k) = k \\bmod M$, how are probe slots generated for iteration $i = 0, 1, 2, 3...$?',
        options: [
          '$(h(k) + i) \\bmod M$',
          '$(h(k) + i^2) \\bmod M$',
          '$(h(k) \\times i) \\bmod M$',
        ...
        correctIndex: 1,
        explanation: 'Quadratic Probing adds $i^2$ offset to jump quadratically across the table and break up primary clusters.',
        examTip: 'Linear Probing = $+i$, Quadratic Probing = $+i^2$, Double Hashing = $+i \\cdot h_2(k)$.'
      },
      {
        id: 'set-q4',
        question: 'What is Primary Clustering in open addressing hash tables?',
        options: [
          'The accumulation of duplicate keys at slot 0',
          'The formation of long contiguous blocks of occupied slots that worsen probe sequence lengths',
          'The allocation of multiple linked lists per bucket slot',
          'The crash caused by dividing by zero in hash functions'
        ],
        correctIndex: 1,
        explanation: 'Linear probing tends to build continuous runs of filled slots. Any key hashing into or near the run gets added to the end, making the cluster even longer.',
        examTip: 'Primary Clustering occurs with Linear Probing; Quadratic Probing and Double Hashing eliminate it.'
      },
      {
        id: 'set-q5',
        question: 'What happens when you insert an existing duplicate element into a std::set?',
        options: [
          'The existing element is overwritten',
          'The insertion is silently ignored and the size remains unchanged',
          'The set throws a runtime exception',
          'A duplicate node is added to the left child'
        ],
        correctIndex: 1,
        explanation: 'std::set enforces strict key uniqueness. Inserting a duplicate key returns pair with `second = false` and leaves the set untouched.',
        examTip: 'If duplicate values are needed while maintaining sorted order, use std::multiset.'
      }
    ]
  },
  {
    ...
    subtitle: 'Associative Key-Value Mappings & Adjacency Graph Architectures',
    icon: 'Compass',
    importance: '🔥 CRITICAL',
    ...
    keyConcepts: [
      ...
      },
      {
        ...
        mathFormula: `Graph Adjacency Map Specification:
$$ \\text{std::map}\\langle \\text{string}, \\text{vector}\\langle \\text{pair}\\langle \\text{string}, \\text{int}\\rangle \\rangle \\rangle \\quad \\text{roadMap;} $$

Example Adjacency Mapping:
$$ \\text{"CityA"} \\to \\{ (\\text{"CityB"}, 1400\\text{ km}), (\\text{"CityC"}, 280\\text{ km}) \\} $$`,
        bulletPoints: [
...
- Finding all neighbors of node u:
  ...
  Adjacency Matrix takes slow O(|V|) time (must scan the whole row).`,
    ...
    quizzes: [
      ...
      {
        id: 'map-q2',
        question: 'For a sparse graph with V = 10,000 vertices and E = 20,000 edges, what is the memory footprint ratio of Adjacency List vs Adjacency Matrix?',
        options: [
          'Adjacency List uses O(V + E) (~30,000 items) vs Matrix using O(V²) (100,000,000 items)',
          'Both use the exact same memory',
          'Adjacency Matrix uses less memory',
          'Adjacency List uses O(V³) memory'
        ],
        correctIndex: 0,
        explanation: 'An Adjacency Matrix requires V² = 100,000,000 cells regardless of edge count, whereas an Adjacency List stores only the actual V + E = 30,000 node pointers.',
        examTip: 'Adjacency List memory = O(V + E); Adjacency Matrix memory = O(V²).'
      },
      {
        id: 'map-q3',
        question: 'What types are stored in each element of a C++ std::map<K, V> during iteration?',
        options: [
          'std::pair<const K, V>',
          'std::vector<V>',
          'Only value V',
        ...
        correctIndex: 0,
        explanation: 'Every node in a std::map holds a `std::pair<const K, V>`. `element.first` is the immutable key, and `element.second` is the associated value.',
        examTip: 'In C++17 ranged for loops: `for (auto const& [key, value] : myMap)` destructures the pair directly.'
      },
      {
        id: 'map-q4',
        question: 'How does iterating over a std::map differ from iterating over a std::unordered_map?',
        options: [
          'std::map iterates in strictly ascending order of keys; std::unordered_map iterates in arbitrary bucket order',
          'std::map is faster to iterate',
          'std::unordered_map iterates in descending order',
          'Neither supports iteration'
        ],
        correctIndex: 0,
        explanation: 'std::map is built on a Red-Black BST, so inorder iteration yields keys in sorted order. std::unordered_map is a hash table whose order depends on hash buckets.',
        examTip: 'If key ordering is required during output, use std::map.'
      },
      {
        id: 'map-q5',
        question: 'Which method safely checks if key K exists in `std::map<K, V> m` without mutating the map if K is missing?',
        options: ['m[K]', 'm.find(K) != m.end()', 'm.get(K)', 'm.at(K) == 0'],
        correctIndex: 1,
        explanation: '`find(K)` returns an iterator to `m.end()` if key is not found, leaving the map unmodified. `m[K]` inserts a default value.',
        examTip: 'Always use find() or count() for safe read-only key existence checks.'
      }
    ]
  }
];
